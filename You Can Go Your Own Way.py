...
class DiffPath():
    def _create_path(self, dimen, existingPath):
        graph = [["0" for i in range(dimen)] for i in range(dimen)]
        takenPath = self._check_path(existingPath)
        temp = []
        for i in range(len(takenPath)):
            temp.append([takenPath[i][1], takenPath[i][0]])
        for i in range(1, len(temp)):
            graph[temp[i][0]][temp[i][1]] = "1"
        for i in graph:
            print(i)

    def _moveSouth(self, a, b):
        return a, b + 1

    def _moveEast(self, a, b):
        return a + 1, b
    ...
    def _check_path(self, path):
        ...
        for i in path:
            if i == "E":
                a, b = self._moveEast(a, b)
                start.append([a, b])
            if i == "S":
                a, b = self._moveSouth(a, b)
                start.append([a, b])
        return start

    def difPath(self, num, path):
        temp = ''
        for i in path:
            if i == 'E':
                temp += "S"
...
# check = DiffPath()
# print(check.difPath(5, "EESSSESE"))
...
for i in range(1, t + 1):
    dimen = int(input())
    path = str(input())
    newPath = new.difPath(dimen, path)
    print("Case #{}: {}".format(i, newPath))
